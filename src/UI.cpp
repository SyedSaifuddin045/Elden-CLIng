...
void UI::PrintPlayerDetails(Window& window)
{
  ...
  E = "HP : "+std::to_string(player_stats.current_HP)+"/"+std::to_string(player_stats.max_HP);
  mvwprintw(win,2,1,"%s",E.c_str());
  E = "STAMINA : "+std::to_string(player_stats.current_Stamina)+"/"+std::to_string(player_stats.max_Stamina);
...
void UI::GeneratePlayGameUI(std::vector<std::string> Options,
  ...
  if (Actions_Window.getWindow()) {
    ...
    if(Selected == Game::ACTION_OPTIONS[0])
    {
      ...
      PlayerTurnString = Game::player.getName() + " attacked the enemy " + Game::current_Enemy.getName() + " for " + std::to_string(damage)+" damage";
      attack_menu.Destroy();
    ...
    if(Selected == Game::ACTION_OPTIONS[2])
    {
      ...
      {
        block = true;
        int BlockedDamage = Game::player.getStats().Defense / 4 + Game::player.getStats().HP_regain;
        Game::player.RecoverHP(BlockedDamage);
        PlayerTurnString += "And blocked " + std::to_string(P.second - BlockedDamage)+" damage";
      }
    }
...
void UI::GenerateMainMenu(std::vector<std::string> &Options) {
  ...
  main_menu.Create(Main_Menu_Window.getWindow(), 1, 0,
                   ...
                   Main_Menu_Window.getWidth(),Main_Menu_Window.getHeight() - 2 ,1, Options);
...
void UI::GenerateCharaterSelect(std::vector<std::string> &Characters) {
  ...
  Character_menu.Create(Character_Select_Window.getWindow(), 1, 0,
                        ...
                        Character_Select_Window.getWidth(),Character_Select_Window.getHeight() - 2,1, Characters);
...
void UI::Clear() {
  for (auto win : {New_Game_Window,Main_Window,Actions_Window,Inventory_Window,Status_Window,Game_Window,Main_Menu_Window,Character_Select_Window,End_Screen}) {
    // win.DestroyMenu();
    win.DestroyWindow();
  }
}
...
void UI::GenerateStatusWindow() {
  ...
  mvwprintw(Status_Window.getWindow(), beg_y + 6, w_max_x - E.size() - 1, "%s", E.c_str());

  E = "DEFENSE:[" + std::to_string(Game::current_Enemy.getStats().Defense) + "]";
  mvwprintw(Status_Window.getWindow(), beg_y + 7, w_max_x - E.size() - 1, "%s", E.c_str());
  ...
  while ((c = wgetch(Game_Window.getWindow())) != KEY_F(1)) {
    ...
    case KEY_DOWN:
      ...
      if (cur_Y + increment >= Game::row_beg + 2 &&
          ...
          Game::isObstacleLocation(newLocation) == false && !Game::isEnemyLocation(newLocation)) {
        cur_Y += increment;
        CurrentLocation.y = cur_Y;
      }
      break;
